...
namespace SoundIdentification.AudioProxy
{
    class SamplesAggregator
    {
        ...
        public float[] ReadSamplesFromSource(ISamplesProvider samplesProvider, double secondsToRead, int sampleRate)
        {
            ...
            int totalBytesToRead = GetTotalBytesToRead(secondsToRead, sampleRate),
            totalBytesRead = 0;
            ...
            while (totalBytesRead < totalBytesToRead)
            {
                ...
                if (bytesRead < 0)
                {
                    throw new Exception("Number of read bytes is negative. Check your audio provider.");
                }
            ...
            {
                throw new Exception($"Could not read requested number of seconds {secondsToRead}, audio file is not that long");
            }
